...
import { Smartphone, CheckCircle2, ArrowRight, Globe, Menu, Download, RefreshCw, AlertTriangle } from "lucide-react";
...
export default function InstallPage() {
  ...
  const [manifestStatus, setManifestStatus] = useState("prüfe...");
  ...
  useEffect(() => {
    ...
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.getRegistration("/").then((reg) => {
        if (reg?.active) setSwStatus("✅ Aktiv");
        else if (reg?.installing) setSwStatus("⏳ Wird installiert...");
        else setSwStatus("❌ Nicht registriert");
      }).catch(() => setSwStatus("❌ Fehler"));
    } else {
    ...
    fetch("/manifest.json").then((r) => {
      setManifestStatus(r.ok ? "✅ Gefunden" : "❌ Nicht gefunden");
    }).catch(() => setSwStatus("❌ Fehler"));
  }, []);
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10">
      <div className="card p-8 text-center">
        ...
        {/* Debug-Panel */}
        <div className="mt-6 rounded-lg border border-line bg-ink/60 p-4 text-left">
          <p className="mb-2 font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
            PWA-Status
          </p>
          <div className="space-y-1 text-xs">
            <p>Service Worker: <span className="text-paper">{swStatus}</span></p>
